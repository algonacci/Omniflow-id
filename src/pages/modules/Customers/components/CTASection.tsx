...
export default function CTASection() {
	...
	const handlePrimaryCTA = () => {
		void trackCTAEvent({
			ctaKey: "customers_cta_schedule_demo",
			locale: currentLang,
			moduleKey: "customers",
		}).catch(() => undefined);
	};
	...
	const handleSecondaryCTA = () => {
		void trackCTAEvent({
			ctaKey: "customers_cta_contact_sales",
			locale: currentLang,
			moduleKey: "customers",
		}).catch(() => undefined);
	};
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
					<div>
						<div className="inline-flex items-center px-4 py-2 bg-blue-800 text-blue-100 rounded-full text-sm font-semibold mb-6">
							...
							{t("customers.cta.badge")}
						</div>
						<h2 className="text-white mb-6">{t("customers.cta.title")}</h2>
						...
						<div className="flex flex-col sm:flex-row gap-4 mb-8">
							...
							>
								{t("customers.cta.contactSales")}
							</Link>
						</div>
						<div className="flex items-center space-x-6 text-blue-100">
							<div className="flex items-center">
								<CheckCircle className="h-4 w-4 mr-2" />
								<span className="text-sm">{t("customers.cta.features.trial")}</span>
							</div>
							<div className="flex items-center">
								<CheckCircle className="h-4 w-4 mr-2" />
								<span className="text-sm">{t("customers.cta.features.setup")}</span>
							</div>
							<div className="flex items-center">
								<CheckCircle className="h-4 w-4 mr-2" />
								<span className="text-sm">
									{t("customers.cta.features.support")}
								</span>
							</div>
						</div>
					</div>
						...
						<div className="card-enterprise p-6 bg-white/10 backdrop-blur-sm border border-white/20">
							<div className="flex items-center space-x-4">
								...
								<div>
									...
									<div className="text-blue-100">
										{t("customers.cta.stats.reduction")}
									</div>
								</div>
							</div>
						</div>
