...
export default function CTASection() {
	...
	const handlePrimaryCTA = () => {
		void trackCTAEvent({
			ctaKey: "assetManagement_cta_schedule_demo",
			locale: currentLang,
			moduleKey: "assetManagement",
		}).catch(() => undefined);
	};

	const handleSecondaryCTA = () => {
		void trackCTAEvent({
			ctaKey: "assetManagement_cta_contact_sales",
			locale: currentLang,
			moduleKey: "assetManagement",
		}).catch(() => undefined);
	};
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
					<div>
						...
						<div className="flex flex-col sm:flex-row gap-4 mb-8">
							...
							>
								{t("assetManagement.cta.scheduleDemo")}
						...
						<div className="flex items-center space-x-6 text-blue-100">
							...
							<div className="flex items-center">
								<CheckCircle className="h-4 w-4 mr-2" />
								<span className="text-sm">{t("assetManagement.cta.features.setup")}</span>
							</div>
							<div className="flex items-center">
								<CheckCircle className="h-4 w-4 mr-2" />
								<span className="text-sm">
									{t("assetManagement.cta.features.support")}
								</span>
							</div>
						</div>
					</div>
