...
const screenshotKeys = ["dashboard", "warehouse", "tracking"];
...
export default function Screenshots() {
	...
	const screenshots = [
		{
			image:
				"https://images.unsplash.com/photo-1553852521-ff0c65d71ca9?auto=format&fit=crop&q=80&w=800",
		},
		{
			image:
			...
			<div className="container-enterprise">
				<div className="text-center mb-16">
					<div className="inline-flex items-center px-4 py-2 bg-sky-100 text-sky-800 rounded-full text-sm font-semibold mb-6">
						...
						{t("inventory.screenshots.badge")}
					</div>
					<h2 className="text-enterprise-primary mb-6">
						{t("inventory.screenshots.title")}
					</h2>
					<p className="text-xl text-enterprise-secondary max-w-3xl mx-auto">
						{t("inventory.screenshots.subtitle")}
					</p>
				</div>

				<div className="space-y-20">
					{screenshotKeys.map((screenshotKey, index) => {
						const features = t(`inventory.screenshots.${screenshotKey}.features`, {
							returnObjects: true,
						});
							...
							>
								<div className={`${index % 2 === 1 ? "lg:col-start-2" : ""}`}>
									<div className="relative">
										<div className="absolute inset-0 bg-gradient-to-r from-sky-600/10 to-blue-700/10 rounded-3xl transform rotate-2"></div>
										<img
											src={screenshots[index].image}
											alt={t(`inventory.screenshots.${screenshotKey}.title`)}
								...
								>
									<div>
										<h3 className="text-3xl font-bold text-enterprise-primary mb-4">
											{t(`inventory.screenshots.${screenshotKey}.title`)}
										</h3>
										<p className="text-xl text-enterprise-secondary leading-relaxed">
											{t(`inventory.screenshots.${screenshotKey}.description`)}
										</p>
									</div>
									...
									<button className="btn-primary">
										{t("inventory.screenshots.exploreFeature")}
									</button>
								</div>
							</div>
						);
